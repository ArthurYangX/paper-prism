...
**v0.1.0** —— 可用，并已在真实论文上跑过。笔记/幻灯/图谱产物和绑定都已稳定；skill 的*内部* prompt、阶段接线和配置键随着持续打磨仍可能调整。测试套件 **184 项检查、零外部依赖**（`python3 tests/test_prism.py`）。见 [CHANGELOG.md](CHANGELOG.md)。
