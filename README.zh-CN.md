...
![tests 186 passing](https://img.shields.io/badge/tests-186%20passing-brightgreen)

[English](README.md) · **简体中文**

<p align="center">
  <img src="examples/showcase/paper-prism-hero.png" width="100%" alt="paper-prism — 7 种输入 → 5 阶段并行流水线（Opus + Sonnet 扇出）→ 三件套绑定的 Obsidian 产物（深度笔记 + 幻灯 + 概念图谱）">
</p>
...
cd paper-prism
./install.sh          # 软链 skill、装时问一次你的 vault 路径、运行依赖 doctor
```

装好就这些。`install.sh` 把 `skills/paper-prism/` 软链进 `~/.claude/skills/`（Claude Code 随即发现它——斜杠命令即 `/paper-prism`），**装的时候问一次你的 Obsidian vault 路径**（唯一必填项；其余全有合理默认），再运行依赖 doctor 检查下列工具。无需手动编辑任何 JSON。
...
模式 1–6 =「你指着自己已有的论文」；**模式 7 =「发现源把论文送到你面前」**——paper-prism 从不抓取、不打分（那留给独立的上游 skill：每日精选、文献检索、Semantic Scholar、arXiv）。它们吐一个 `{title, arxiv?, score?, why?}` 的 JSON 列表（或 `.bib`）；paper-prism 收进队列、折射留下的那些。参考文献、`.bib`、发现源都经 `prism_refs.py` 转成队列：
...
<p align="center">
  ...
  <img src="examples/showcase/run-attention/preview/mamba-02-overview.png" width="49%" alt="Mamba deck — Selective SSM 架构页">
</p>
...
deck 流水线（`make a deck`）跑**五个阶段，默认并行**——上面的「一图速览」已展示。主 agent 充当 coordinator：先把活扇给三个 subagent（阶段 2），再复核它们的产出、组装笔记 + deck（阶段 3）——`main → subagents → main`。批量（文件夹 / Zotero / YAML）在外面套一个 `/loop` 主控 prompt：扫描 → 按已有产物去重 → 每轮派出 `parallel` 个 paper-coordinator → 队列空则停止。失败写入错误日志，**绝不阻塞**下一篇。逐阶段细节与耗时见 `docs/architecture.md`。

---

## 配置

你已经配置好了——`install.sh` 创建了 `config.json` 并设好了你的 `vault_path`。**本节是可选的微调。**（`config.json` 已 gitignore——它保存你的私有 vault 路径；重跑 `install.sh` 绝不覆盖它。）配置按以下顺序解析，命中即止：
...
**v0.1.0** —— 可用，并已在真实论文上跑过。笔记/幻灯/图谱产物和绑定都已稳定；skill 的*内部* prompt、阶段接线和配置键随着持续打磨仍可能调整。测试套件 **186 项检查、零外部依赖**（`python3 tests/test_prism.py`）。见 [CHANGELOG.md](CHANGELOG.md)。
